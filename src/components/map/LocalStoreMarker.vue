...
<template>
  <div @click.stop="emit('click')" class="relative">
    <div
      class="absolute top-0 left-0 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center"
    >
      <div v-show="mapLevel <= 13" class="flex flex-col items-center">
        <svg
          width="25.8"
          height="25.8"
      ...
      <div v-if="mapLevel <= 9" class="Body03 text-center leading-tight truncate whitespace-nowrap">
        {{ storeName }}
      </div>
      ...
      >
        <img
          ...
          :src="mapMarker"
          alt="Selected Marker"
          class="absolute top-[-3rem] left-1/2 w-[40px] h-[40px] z-[50] pointer-events-none transform -translate-x-1/2"
        />
      </transition>
    </div>
  </div>
